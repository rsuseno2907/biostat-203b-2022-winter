...
```
```{r}
a <- mutate(mimic_icu_cohort, datime = yday(deathtime) - yday(admittime)) %>% 
  mutate(z = replace_na(datime,0)) %>%  #replace all NAs in datime
  mutate(datime = replace(z, z < -300, z + 365))
```
