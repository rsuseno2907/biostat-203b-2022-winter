...
admissions_tble <- select(admissions_tble, -c(16:21))
admissions_tble
```
...
```
The graph above has already been zoomed so that we can observe the tip of the bar plot. Based on the look of it, Saturday seems to have the highest admission day among others.
...
admissions_tble %>% 
  ...
  # coord_cartesian(ylim = c(70000, 80000)) 
```
Based on my observation, most patients are admitted in the evening or at night, peaking at midnight, as opposed to morning/afternoon.
...
patients_tble <- inner_join(patients_tble, icustays_tble, by="subject_id")
patients_tble <- select(patients_tble, -c(7:13))
patients_tble
```
...
#by_anchor <- group_by(patients_tble, anchor_age)
#by_anchor
```
...
print(width = Inf)
```
In `labevents_filtered_itemid.csv.gz` there are 30,198,826 rows. Only joining them with icustays_tble according to the subject_id gave us 16,698,462 rows.
...
labevents_tble <- labevents_tble %>%  
  filter(itemid == c(50912, 50971, 50983, 50902, 50882, 51221, 
                     51301, 50931, 50960, 50893)) %>% 
  print(width = Inf)
```
```{r}
by_id2 <- group_by(labevents_tble, subject_id)
by_id2
```
After filtering only with the itemid of interests, we get 52,282 different subject ID, which means not all patients got these 10 tests, therefore reducing the number of distinct patient from 53,907 to 52,282.
...
chartevents_tble <- 
  read_csv("/mnt/mimiciv/1.0/icu/chartevents_filtered_itemid.csv.gz",
     ...
     lazy = TRUE) %>%
semi_join(icustays_tble, by = c("subject_id")) %>%
filter(itemid == c(220045, 220181, 220179, 223761, 220210)) %>% 
print(width = Inf)
```
