...
labevents_tble <- tbl(con, "labevents") %>% 
  ...
  semi_join(icustays_tble, by = "subject_id") %>% #filter only the ones that appear in icustays
  filter(itemid %in% c(50971, 50912, 50983, 50902, 50882, 
                       51221, 51301, 50931, 50960, 50893)) %>%
  left_join(select(icustays_tble, subject_id, intime, outtime),
            by = "subject_id") %>%  #add intime n outtime
  print(width=Inf)
```
...
labevents_tble <- labevents_tble %>%
  filter(charttime > intime & charttime < outtime) %>% #make sure charttime is between in and out time
  group_by(subject_id, itemid) %>% 
  slice_min(n = 1, order_by = charttime) %>% #take first lab measurement 
  ungroup() %>% 
  select(-charttime, -intime, -outtime) %>% #remove unnecessary variables between itemid and valuenum
  pivot_wider(names_from = itemid, 
              values_from = valuenum, 
              names_prefix = 'l') %>%  #pivot lab items
  rename(creatinine = "l50912", potassium = "l50971", sodium = "l50983",
       chloride = "l50902", bicarbonate = "l50882", hematocrit = "l51221",
       wbc_count = "l51301", glucose = "l50931",
       magnesium = "l50960", calcium = "l50893") %>% 
  print(width = Inf)
```
...
chartevents_tble <- chartevents_tble %>%
  ...
  pivot_wider(names_from = itemid, 
              values_from = valuenum, 
              names_prefix = 'c') %>%  #pivot lab items
  rename(heart_rate = "c220045", mean_non_invasive_bp = "c220181",
       systolic_non_invasive_bp = "c220179", body_temp = "c223761",
       resp_rate = "c220210") %>%
  print(width = Inf)
```
...
icu_cohort <- icustays_tble %>% 
  ...
  mutate(shinda = date_diff(dod, admittime, DAY)) %>% 
...
```{r}
#icu_cohort %>% 
#  select(admittime, dod, thirty_days_mort) %>% 
#  filter(thirty_days_mort == TRUE) %>% 
#  summarise(n=n()) %>% 
#  print(width = Inf)
```
