...
labevents_tble <- labevents_tble %>%
  ...
  pivot_wider(names_from = itemid, 
              ...
              names_prefix = 'lab') %>%  #pivot lab items
  print(width = Inf)
```
...
chartevents_tble <- chartevents_tble %>%
  group_by(subject_id, itemid) %>% 
  slice_min(n=1, order_by = intime) %>% #take first lab measurement 
  ungroup() %>% 
  pivot_wider(names_from = itemid, values_from = valuenum, names_prefix = 'chart') %>%  #pivot lab items
  print(width = Inf)
```
