...
Prepare data for imputation to only include numerical values (specifically, I only include lab measurements, chart measurements, and age of patients).
```{r}
tbi <- select(mimic_icu_cohort, c( 'gender','age_hadm','marital_status',
                                   'ethnicity','thirty_days_mort',23:37))
summary(tbi)
```
...
saveRDS(imputedObj, "imputation_result") 
```
...
print(imp_icustays_tble, width=Inf)
```
