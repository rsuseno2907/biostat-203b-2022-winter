...
Replace outliers with NAs, then use `summary()` to check whether outlier has been removed.
```{r}
...
**Answer**
I'm setting the cell below to `eval=F` because running `miceRanger()` will take forever to knit.
```{r, eval = F}
seqTime <- system.time(
...
**Answer**
This plot demonstrates how the distribution of the imputed data differ from the original distribution. The original data is plotted as red and the imputed data is plotted in black. As we can observe, they all fall within a normal distribution and are generally follow the same trend between the original and the imputed data.
```{r}
plotDistributions(imputedObj, vars = 'allNumeric')
```


This diagnostic plot tells us the accuracy of classification and r-squared regression.
```{r}
...
The accuracy for logistic regression: `glm()`
```{r}
...
The accuracy for logistic regression with LASSO penalty: `glmnet()`
```{r}
...
When comparing the two performance, we can see that the accuracy for logistic regression with LASSO penalty is a little bit worse than logistic regression in this case. They are still very fairly comparable as they are usually both around 90% accurate. The numbers are different everytime simply because we are splitting the training/testing set at 'random', so the performance will be different every run.